...
for row in range(2,sheet.max_row + 1):
    if sheet['A' + str(row)].value != 'Trial Card No' or 'FOR OFFICIAL USE ONLY':
        if sheet['H' + str(row)].value != '':
           ...
           # Combined singleton values
            ...
            tc_Status.setdefault(stat, {'Pri STARRED': 0, 'Pri 1S': 0, 'Pri 1': 0, 'Pri 2S': 0,   'Pri 2': 0, 'Pri 3S': 0, 'Pri OTHER': 0, 'Total': 0})
            # By status and screen and screening count
            tc_Stat_Scrn.setdefault(stat, {})
            tc_Stat_Scrn[stat].setdefault(scrn, {})
            tc_Stat_Scrn[stat][scrn].setdefault(scrngs, {'Pri STARRED': 0, 'Pri 1S': 0, 'Pri 1': 0, 'Pri 2S': 0,   'Pri 2': 0, 'Pri 3S': 0, 'Pri OTHER': 0, 'Total': 0})

            # Update the Priority and Status dictionary counts
            tc_Status[stat]['Total'] += 1
            tc_Stat_Scrn[stat][scrn][scrngs]['Total'] +=  1
            if myPri_star == True:
                tc_Status[stat]['Pri STARRED'] +=  1
                tc_Stat_Scrn[stat][scrn][scrngs]['Pri STARRED'] +=  1            
            elif myPri_1s == True:
                tc_Status[stat]['Pri 1S'] += 1
                tc_Stat_Scrn[stat][scrn][scrngs]['Pri 1S'] += 1
            elif myPri_1 == True:
                tc_Status[stat]['Pri 1'] += 1
                tc_Stat_Scrn[stat][scrn][scrngs]['Pri 1'] += 1
            elif myPri_2s == True:
                tc_Status[stat]['Pri 2S'] += 1
                tc_Stat_Scrn[stat][scrn][scrngs]['Pri 2S'] += 1
            elif myPri_2 == True:
                tc_Status[stat]['Pri 2'] += 1
                tc_Stat_Scrn[stat][scrn][scrngs]['Pri 2'] += 1
            elif myPri_3s == True:
                tc_Status[stat]['Pri 3S'] += 1
                tc_Stat_Scrn[stat][scrn][scrngs]['Pri 3S'] += 1
            elif myPri_other == True:
                tc_Status[stat]['Pri OTHER'] += 1
                tc_Stat_Scrn[stat][scrn][scrngs]['Pri OTHER'] += 1
            else:
                print('Row read error with Status of Open, Priority in not Valid. Row: ' + str(row) + ' TC Number: ' + dsp)
    elif sheet['A' + str(row)].value == 'Trial Card No':
